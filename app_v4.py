...
def get_player_stats_and_elo(df, all_members=None):
    ...
    if '경기ID' in df.columns: df = df.drop_duplicates(subset=['경기ID'], keep='last')
...
def generate_league_schedule(attendees, target_games, mode, stats):
    ...
    total_rounds = math.ceil((len(attendees) * target_games) / (courts_num * 4))
    past_pairs = set()
...
def draw_bracket_plot(teams_4):
    ...
    sf_1, sf_2 = teams_4[0], teams_4[1]
    sf_3, sf_4 = teams_4[2], teams_4[3]
    f1, f2 = winners.get('semi_1', '???'), winners.get('semi_2', '???')
    champion = winners.get('final', '최종 우승')
    fig, ax = plt.subplots(figsize=(10, 5))
    ax.axis('off')
    def db(x, y, t, c='#f0f2f6', h=False):
        ax.text(x, y, t, ha='center', va='center', fontsize=9, bbox=dict(boxstyle="round,pad=0.3", fc=('gold' if h else c), ec='black'))
    db(0.2, 0.8, sf_1, h=(sf_1==f1)); db(0.2, 0.6, sf_2, h=(sf_2==f1))
    db(0.2, 0.4, sf_3, h=(sf_3==f2)); db(0.2, 0.2, sf_4, h=(sf_4==f2))
    db(0.5, 0.7, f1, c='lightblue', h=(f1==champion)); db(0.5, 0.3, f2, c='lightblue', h=(f2==champion))
    db(0.8, 0.5, champion, c='gold')
    return fig
...
# [0] 실시간 현황판 (다크모드 포함 원본 로직)
if menu == "📺 실시간 현황판":
    st.header("📺 LIVE SCOREBOARD")
    c1, c2 = st.columns([0.8, 0.2])
    with c2: view_mode = st.radio("화면 모드", ["🌞 라이트", "🌙 다크"], label_visibility="collapsed")
    bg, tx, card = ("#1E1E1E", "#FFFFFF", "#2D2D2D") if view_mode == "🌙 다크" else ("#FFFFFF", "#000000", "#F0F2F6")
    live_df = load_data("실시간현황")
    if not live_df.empty:
        for r in live_df['라운드'].unique():
            st.markdown(f"### 📌 {r}")
            for _, row in live_df[live_df['라운드']==r].iterrows():
                st.markdown(f"""<div style="background-color:{card}; padding:15px; border-radius:10px; margin-bottom:10px; text-align:center; color:{tx};">
                    <b>{row['팀1']}</b> VS <b>{row['팀2']}</b><br><small>🏟️ {row['코트']}</small></div>""", unsafe_allow_html=True)

# [1] 회원 관리 (원본 삭제 기능 포함)
elif menu == "👥 회원 관리":
    st.header("👥 회원 관리")
    member_df = load_data("회원정보", ["이름", "가입일", "메모"])
    t1, t2 = st.tabs(["📜 목록", "⚙️ 관리"])
    with t1: st.dataframe(member_df, use_container_width=True)
    with t2:
        if is_admin:
            with st.form("add"):
                n, m = st.text_input("이름"), st.text_input("메모")
                if st.form_submit_button("등록"): add_member_to_db(n, m); st.rerun()

# [2] 경기 운영 (탭 독립 렌더링 + 회장맘대로 추가)
elif menu == "🏟️ 경기 운영":
    tabs = st.tabs(["🔄 일반 매칭", "🏆 토너먼트", "⚔️ 팀 대항전", "🔢 KDK", "👑 회장맘대로"])
    member_df = load_data("회원정보", ["이름"])
    names = member_df["이름"].tolist() if not member_df.empty else []
    ...
    with tabs[0]:
        ...
        att = st.multiselect("출석 체크", names, key="att_n")
        c1, c2 = st.columns(2)
        tg = c1.slider("게임 수", 1, 6, 3, key="tg_n")
        mm = c2.radio("방식", ["🎲 랜덤 복식", "⚖️ ELO밸런스"], key="mm_n")
        if st.button("🚀 대진표 생성", key="btn_n"):
            st.session_state.schedule = generate_league_schedule(att, tg, mm, get_player_stats_and_elo(load_data("경기기록"), names))
            st.session_state.is_generated = True
            save_schedule_backup(st.session_state.schedule)
        
        if st.session_state.get('is_generated'): # 출력부 봉인
            st.divider()
            for rd in st.session_state.schedule:
                ...
                for i, m in enumerate(rd['matches']):
                    with st.container(border=True):
                        ...
                        s1, s2 = col2.number_input("점1", key=f"s1_{rd['round_num']}_{i}", min_value=0), col3.number_input("점2", key=f"s2_{rd['round_num']}_{i}", min_value=0)
                        key = f"rec_n_{rd['round_num']}_{i}"
    ...
    with tabs[4]:
        ...
        b_att = st.multiselect("출석 체크", names, key="att_b")
        c1, c2 = st.columns(2)
        b_gc = c1.slider("게임 수", 3, 10, 4, key="gc_b")
        b_opt = c2.radio("방식", ["고정 팀(나머지 랜덤)", "완전 수동"], key="opt_b")
        ...
        if b_opt == "고정 팀(나머지 랜덤)":
            with st.expander("📌 고정 팀 설정"):
                for i in range(1, 5):
                    tc = st.columns(2)
                    p1 = tc[0].selectbox(f"{i}코트-1", ["미지정"]+b_att, key=f"p1_{i}")
                    p2 = tc[1].selectbox(f"{i}코트-2", ["미지정"]+b_att, key=f"p2_{i}")
                ...
                for g in range(1, b_gc+1):
                    ...
                    for ft in fixed:
                        if len(t_rem)>=2:
                            ...
                            ms.append({"t1": ft, "t2": f"{t_rem.pop(0)}, {t_rem.pop(0)}", "court": "코트"})
                    new_b.append({"game": g, "matches": ms})
            else:
                for g in range(1, b_gc+1):
                    new_b.append({"game": g, "matches": [{"t1": "미지정", "t2": "미지정", "court": "코트"} for i in range(len(b_att)//4)]})
            st.session_state.boss_schedule = new_b
            st.session_state.boss_active = True
            save_schedule_backup(new_b); st.rerun()

        if st.session_state.get('boss_active'): # 출력부 봉인
            st.divider()
            for g in st.session_state.boss_schedule:
                ...
                for i, m in enumerate(g['matches']):
                    with st.container(border=True):
                        ...
                        if b_opt == "완전 수동":
                            t1 = c1.multiselect("팀1", b_att, max_selections=2, key=f"bt1_{g['game']}_{i}")
                            t2 = c1.multiselect("팀2", b_att, max_selections=2, key=f"bt2_{g['game']}_{i}")
                            m['t1'], m['t2'] = ", ".join(t1), ", ".join(t2)
                        else: c1.write(f"{m['t1']} vs {m['t2']}")
                        s1, s2 = c2.number_input("점1", key=f"bs1_{g['game']}_{i}", min_value=0), c3.number_input("점2", key=f"bs2_{g['game']}_{i}", min_value=0)
                        k = f"btn_b_{g['game']}_{i}"
                        if is_admin and st.button("기록", key=k, disabled=(k in st.session_state.get('recorded_ids', set()))):
                            add_match_record(m['t1'], m['t2'], s1, s2)
                            if 'recorded_ids' not in st.session_state: st.session_state.recorded_ids = set()
                            st.session_state.recorded_ids.add(k); st.rerun()
