...
# --- 설정 및 한글 폰트 ---
st.set_page_config(page_title="행님표 테니스 ERP V5.0 (Google Sheet)", page_icon="🎾", layout="wide")

# [폰트 설정 - 스트림릿 클라우드 리눅스 환경 호환]
# 로컬에서는 Malgun Gothic이 좋지만, 서버에는 없을 수 있어서 기본 설정 유지 혹은 나눔고딕 등 설치 필요
# 여기서는 에러 방지를 위해 try-except 처리
try:
...
@st.cache_data(ttl=60) # 60초 캐싱으로 속도 향상, 데이터 변경시 자동 초기화
def load_data(sheet_name, expected_headers):
    """구글 시트에서 데이터를 읽어와서 DataFrame으로 변환"""
    try:
        ws = get_or_create_worksheet(sheet_name, expected_headers)
        data = ws.get_all_records()
        df = pd.DataFrame(data)
        
        # 데이터가 비어있어도 컬럼은 존재해야 함
        if df.empty:
...
def add_member_to_db(name, memo):
    ...
    st.cache_data.clear() # 캐시 초기화 (새 데이터 반영)
...
def delete_match_records(match_ids_to_delete):
    ...
    # 경기ID가 숫자로 들어올 수 있으므로 문자로 변환하여 비교
    ...
    # 시트 초기화 후 다시 쓰기 (gspread는 행 삭제가 까다로워서 덮어쓰기가 안정적)
    ws.clear()
    ws.append_row(["날짜", "경기ID", "팀1", "팀2", "점수1", "점수2", "승리팀"]) # 헤더 복구
    if not new_df.empty:
        ws.update([new_df.columns.values.tolist()] + new_df.values.tolist()) # 전체 업데이트 (헤더 포함될 수 있어 주의)
        # update는 헤더 포함해서 덮어쓰므로 아래 방식 사용
        ws.clear()
        ws.update([new_df.columns.values.tolist()] + new_df.values.tolist())
        
    st.cache_data.clear() # 캐시 초기화

# --- [핵심] Elo 포인트 계산 로직 (행님 코드 유지) ---
...
# --- UI 시작 ---
st.title("🎾 행님표 ERP V5.0 [Google Sheet 연동]")
st.caption("고수를 잡으면 점수 대박! 양민 학살은 점수 찔끔! 공정한 실력 시스템 (Data by Google Cloud)")
